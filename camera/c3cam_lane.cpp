...
// === 新增：车道线 ROI  ===
std::vector<CameraROI> camera_lane_rois;  // 新增：每个摄像头的车道线 ROI
...
ROIMode current_roi_mode = ROIMode::VEHICLE;  // 当前编辑模式，新增
...
// === 新增：车道线检测函数（简单判断实线/虚线） ===
bool detect_lane_line(const cv::Mat& frame, const CameraROI& roi){
    if(roi.polygon.empty()) return false;

    // 提取 ROI 内灰度图
    cv::Mat mask = cv::Mat::zeros(frame.size(), CV_8UC1);
    std::vector<std::vector<cv::Point>> poly{roi.polygon};
    cv::fillPoly(mask, poly, cv::Scalar(255));
    cv::Mat gray; cv::cvtColor(frame, gray, cv::COLOR_BGR2GRAY);
    cv::Mat roi_img; gray.copyTo(roi_img, mask);

    // 边缘检测
    cv::Mat edges;
    cv::Canny(roi_img, edges, 50, 150);
    ...
    // 分析线段连续性来判断实线/虚线
    int total_len=0, gap_len=0;
    std::sort(lines.begin(), lines.end(), [](cv::Vec4i a, cv::Vec4i b){ return a[0]<b[0]; });
    int last_x=-1;
    for(auto &l: lines){
        int x1=l[0], x2=l[2];
        int seg_len = x2-x1;
        total_len += seg_len;
        if(last_x>=0) gap_len += x1-last_x;
        last_x = x2;
    }

    if(total_len == 0) return false; // 没检测到线
    double ratio = double(gap_len)/double(total_len);
    return ratio < 0.5; // 新增：ratio小于0.5判定为实线，否则虚线
}
...
void inference_thread(int cam_id) {
    while (running) {
        ...
        if(cam_id < camera_lane_rois.size()){
            ...
            std::cout << "Camera " << cam_id << " lane line is " << (solid?"solid":"dashed") << std::endl;
        }
...
// === 新增 ===
std::mutex lane_roi_file_mutex;

void save_lane_rois_threadsafe(const std::string& filename){
    std::lock_guard<std::mutex> lock(lane_roi_file_mutex);
    std::ofstream ofs(filename);
    for(auto& roi: camera_lane_rois){
        for(auto& pt: roi.polygon) ofs << pt.x << " " << pt.y << " ";
        ofs << "\n";
    }
}
void load_lane_rois(const std::string& filename){
    std::ifstream ifs(filename);
    if(!ifs.is_open()) return;
    camera_lane_rois.clear();
    std::string line;
    while(std::getline(ifs,line)){
        CameraROI roi;
        std::istringstream iss(line);
        int x,y;
        while(iss>>x>>y) roi.polygon.push_back(cv::Point(x,y));
        camera_lane_rois.push_back(roi);
    }
}
...
// === 新增：鼠标回调统一处理函数 ===
void universal_mouse_callback(int event, int x, int y, int flags, void* userdata){
    int cam_id = *reinterpret_cast<int*>(userdata);
    if(cam_id < 0 || cam_id >= cam_max_num) return;

    CameraROI* roi_ptr = nullptr;
    if(current_roi_mode == ROIMode::VEHICLE) roi_ptr = &camera_rois[cam_id];
    else if(current_roi_mode == ROIMode::LANE) roi_ptr = &camera_lane_rois[cam_id];
    ...
    if (changed) {
        auto now = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last_save_time).count() > 500) {
            if(current_roi_mode == ROIMode::VEHICLE) save_rois_threadsafe("rois.txt");
            else save_lane_rois_threadsafe("lane_rois.txt");
            last_save_time = now;
        }
    }
}
    ...
    while (running) {
        if(show_video)
        {
            ...
            for (int i = 0; i < shared_images.size(); ++i) {
                ...
                // === 修改：统一鼠标回调为通用回调，支持车辆/车道 ROI ===
                cv::setMouseCallback(window_name, universal_mouse_callback, &cam_ids[i]);

                // === 新增：绘制 ROI（根据当前模式选择车辆或车道 ROI） ===
                if(current_roi_mode == ROIMode::VEHICLE && i < camera_rois.size() && !camera_rois[i].polygon.empty()) {
                    cv::polylines(shared_images[i], std::vector<std::vector<cv::Point>>{camera_rois[i].polygon}, true, cv::Scalar(0,255,0), 2);
                } else if(current_roi_mode == ROIMode::LANE && i < camera_lane_rois.size() && !camera_lane_rois[i].polygon.empty()) {
                    cv::polylines(shared_images[i], std::vector<std::vector<cv::Point>>{camera_lane_rois[i].polygon}, true, cv::Scalar(255,0,0), 2);
                }
            }

            // === 新增：键盘切换 ROI 模式 ===
            int key = cv::waitKey(1);
            if(key == 'v') current_roi_mode = ROIMode::VEHICLE;
            if(key == 'l') current_roi_mode = ROIMode::LANE;
            if(key == 27) { // ESC
