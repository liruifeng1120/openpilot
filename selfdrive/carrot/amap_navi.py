...
class AmapNaviServ:
                  ...
                  try:
                    ...
                    if device_type == "lidar" or device_type == "camera":  # 雷达模块
                      ...
                      if info.get("right_blind", False):
                        right_blind = True
                      #获取雷达距离数据
                      self.shared_data.lf_drel[lidar_id] = info.get("lf_drel", None)
                      self.shared_data.lb_drel[lidar_id] = info.get("lb_drel", None)
                      self.shared_data.rf_drel[lidar_id] = info.get("rf_drel", None)
                      self.shared_data.rb_drel[lidar_id] = info.get("rb_drel", None)
                      self.shared_data.lf_xrel[lidar_id] = info.get("lf_xrel", None)
                      self.shared_data.lb_xrel[lidar_id] = info.get("lb_xrel", None)
                      self.shared_data.rf_xrel[lidar_id] = info.get("rf_xrel", None)
                      self.shared_data.rb_xrel[lidar_id] = info.get("rb_xrel", None)
  ...
  def make_navi_message(self):
    ...
    if self.sm.alive['controlsState']:
      ...
      msg['active'] = True if control_state.active else False
      ...
      if control_state.active and not msg.get('engaged', False):
        msg['engaged'] = True
