...
class CarrotPlanner:
  ...
  def get_smooth_t_follow(self, v_ego, personality=log.LongitudinalPersonality.standard):
      """
      基于车速的平滑跟车距离调节，利用现有的四档配置参数
      """
      # 车速节点 (m/s): 0, 8, 18, 30 对应 0, 30, 65, 108 km/h
      speed_points = [0.0, 8.0, 18.0, 30.0]

      t_follow_points = [
          self.tFollowGap1,  # 低速
          self.tFollowGap2,  # 中低速
          self.tFollowGap3,  # 中高速
          self.tFollowGap4   # 高速
      ]

      smooth_t_follow = np.interp(v_ego, speed_points, t_follow_points)

      self.jerk_factor = 1.0 if self.myDrivingMode == DrivingMode.Safe else 0.7
      if personality == log.LongitudinalPersonality.aggressive:
          self.jerk_factor = 1.0 if self.myDrivingMode == DrivingMode.Safe else 0.5

      return smooth_t_follow
