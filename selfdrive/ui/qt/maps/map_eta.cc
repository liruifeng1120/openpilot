...
void MapETA::updateETA(float s, float s_typical, float d) {
  ...
  // 翻译距离单位
  QString distanceUnit = uiState()->scene.is_metric ? "公里" : "英里";
  QString distanceValue = distance.first;
  ...
  eta_doc.setHtml(QString(R"(<body><table><tr style="vertical-align:bottom;"><td><b>%1</b></td><td>%2</td>
                      ...
                      .arg(eta.first, eta.second, color, remaining.first, remaining.second, distanceValue, distanceUnit));
