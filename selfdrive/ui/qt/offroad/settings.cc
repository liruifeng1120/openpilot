...
DevicePanel::DevicePanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  offroad_btn = new QPushButton(tr("强制上下线"));
  offroad_btn->setObjectName("init_btn");
  init_layout->addWidget(offroad_btn);
  ...
  // Error Troubleshoot
  QPushButton* errorBtn = new QPushButton(tr("错误日志"));
  errorBtn->setObjectName("init_btn");
  init_layout->addWidget(errorBtn);
  connect(errorBtn, &QPushButton::clicked, [&]() {
    QString content = "";
    QFile errorFile("/data/community/crashes/error.txt");
    QFileInfo fileInfo(errorFile);
    
    if (fileInfo.exists()) {
      QDateTime modifiedTime = fileInfo.lastModified();
      content = modifiedTime.toString("yyyy-MM-dd hh:mm:ss ");
    }
    ...
    ConfirmationDialog::rich(content, this);
  });
...
void DevicePanel::forceoffroad() {
  if (!uiState()->engaged()) {
    if (params.getBool("ForceOffroad")) {
      if (ConfirmationDialog::confirm(tr("确认强制上线?"), tr("强制上线"), this)) {
        // Check engaged again in case it changed while the dialog was open
        if (!uiState()->engaged()) {
      ...
      if (ConfirmationDialog::confirm(tr("确认强制下线?"), tr("强制下线"), this)) {
        // Check engaged again in case it changed while the dialog was open
        if (!uiState()->engaged()) {
    ...
    ConfirmationDialog::alert(tr("要先解除控车"), this);
  }
...
void DevicePanel::updateLabels() {
  ...
  offroad_btn->setText(force_offroad_param ? tr("强制上线") : tr("强制下线"));
...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  startToggles->addItem(new CValueControl("BYDSnG", "BYD自动跟车", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDForceTorqueFix", "BYD过矩修复", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDHandsoffFix", "BYD离手修复", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDForceLowTorqueFactor", "BYD低扭修复", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDReverseAccCAN", "还原BYD ACC", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDReverseBsmCAN", "还原BYD盲区监测系统", "0: 关闭, 1: 开启", 0, 1, 1));
  startToggles->addItem(new CValueControl("BYDAngleControlOffset", "BYD角度控制偏移量", "1 to 0.1", -50, 50, 1));
  startToggles->addItem(new CValueControl("DisableMADS", "关闭全时置中", "0:开启, 1:关闭", 0, 1, 1));
