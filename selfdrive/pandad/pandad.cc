...
void pandad_run(std::vector<Panda *> &pandas) {
  ...
  Panda *peripheral_panda = pandas[pandas.size() - 1];
  bool engaged = false;
  Params params;
  ...
  while (!do_exit && check_all_connected(pandas)) {
    ...
    // dump panda buffer every 5s
    if (rk.frame() % 500 == 0) {
      for (const auto& panda : pandas){
        auto health_opt = panda->get_state();
        if (health_opt) {
          uint32_t dump_buffer = panda->get_dump_buffer();
          if (dump_buffer) {
            params.put("PDDumpBuffer", std::to_string(dump_buffer));
          } else {
            auto p_dump_buffer = std::atoi(params.get("PDDumpBuffer").c_str());
            if (p_dump_buffer != 0) {
              panda->set_dump_buffer(p_dump_buffer);
            }
          }
        }
      }
