...
void Panda::set_dump_buffer(uint32_t dump_buffer) {
  handle->control_write(0xc7, dump_buffer & 0xFFFF, dump_buffer >> 16);
}

uint32_t Panda::get_dump_buffer() {
  uint32_t dump_buffer = 0;
  handle->control_read(0xc8, 0, 0, (unsigned char*)&dump_buffer, sizeof(dump_buffer));
  return dump_buffer;
}
