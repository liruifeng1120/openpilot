...
allowed_system_libs = {
  ...
  "Qt5Qml", "Qt5QuickWidgets", "Qt5Location", "Qt5Positioning",
  "ssl", "crypto", "OpenCL", "wayland-client",
  "curl",
...
# Qt build environment (legacy Qt UI, ported from cp-pc)
qt_env = env.Clone()
...
if arch == "Darwin":
  ...
  if arch == "larch64":
    ...
    if os.path.isdir("#third_party/qt5/larch64/bin"):
...
qt_flags = [
  ...
  # legacy fork code is not -Werror clean
  "-Wno-error=unused-result",
  "-Wno-error=shadow=compatible-local",
  "-Wno-error=shadow=local",
]
qt_env['CXXFLAGS'] += qt_flags
qt_env['LIBPATH'] += ['#openpilot/selfdrive/ui', f"#third_party/maplibre-native-qt/{arch}/lib"]
qt_env['RPATH'] += [Dir(f"#third_party/maplibre-native-qt/{arch}/lib").srcnode().abspath]
qt_env['LIBS'] = qt_libs

Export('env', 'arch', 'qt_env', 'ffmpeg_libs')
...
# Blind spot camera (cam_blindspot)
