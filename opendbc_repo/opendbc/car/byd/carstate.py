...
class CarState(CarStateBase):
    ...
    def update(self, can_parsers) -> structs.CarState: # type: ignore
        ...
        ret.aEgo = cp.vl["AXAY"]["Ax"] - cp.vl["AXAY"]["AxOffset"]    # 纵向加速度
        ret.aEgoY = cp.vl["AXAY"]["AY"] - cp.vl["AXAY"]["AYOffset"]  # 横向加速度
        ret.yawRate = cp.vl["YAW_RATE"]["YawRate"] - cp.vl["YAW_RATE"]["YawRateOffset"] #偏航率
        ...
        # EPS警告检测 - 可通过tuning.py禁用
        if Tuning.DISABLE_EPS_WARNING:
            self.eps_warning = False
        else:
        ...
        # 驻车制动检测 - 可通过tuning.py禁用
        if Tuning.DISABLE_PARKBRAKE:
        ...
        # EPS故障检测 - 可通过tuning.py禁用
        if Tuning.DISABLE_EPS_TEMPORARY_FAULT:
        ...
        # EPS永久性故障检测 - 可通过tuning.py禁用
        if Tuning.DISABLE_EPS_PERMANENT_FAULT:
