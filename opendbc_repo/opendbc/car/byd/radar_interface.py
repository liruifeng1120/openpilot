...
class RadarInterface(RadarInterfaceBase):
    def __init__(self, CP):
        ...
        # ---------- 基础 CAN ----------
        messages = [
            ("RADAR_MRR", 60),  # 雷达消息，60Hz频率
        ]
        try:
            print(f"Initializing CANParser with DBC: {DBC[CP.carFingerprint][Bus.pt]}")
            print(f"Messages: {messages}")
            print(f"Bus: {CanBus.MPC}")
            self.rcp = CANParser(DBC[CP.carFingerprint][Bus.pt], messages, CanBus.MPC)
            print(f"CANParser initialized successfully: {type(self.rcp)}")
            print(f"CANParser has update_strings: {hasattr(self.rcp, 'update_strings')}")
        except Exception as e:
            print(f"CANParser initialization failed: {e}")
            self.rcp = None
