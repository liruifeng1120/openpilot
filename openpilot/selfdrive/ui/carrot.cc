    ...
    void draw(const UIState* s) {
        ...
        if (softHoldActive || brakeHoldActive || carrotCruise) {
            ...
            if (xState == 3 || xState == 5) {      //XState.e2eStop, XState.e2eStopped
                if (v_ego < 1.0) {
                    sprintf(str, "%s", (trafficState >= 1000) ? "信号错误": "信号就绪");
                    ui_draw_text(s, x, disp_y, str, disp_size, COLOR_WHITE, BOLD);
                }
                else {
                    ui_draw_text(s, x, disp_y, "信号减速", disp_size, COLOR_WHITE, BOLD);
                }
#if 0
...
public:
    void draw(const UIState* s) {
        ...
        // Camera publishes amapNavi; prefer its blind flags to render camera-only BSD.
        // Keep a short hold time so UI doesn't flicker on brief message loss or detector jitter.
        constexpr double CAMERA_HOLD_S = 0.5; // hold camera blind state for 0.5s
        static bool last_cam_left = false;
        static bool last_cam_right = false;
        static uint64_t last_cam_left_t = 0;
        static uint64_t last_cam_right_t = 0;

        if (sm.alive("amapNavi")) {
            auto amap = sm["amapNavi"].getAmapNavi();
            // amap leftBlind/rightBlind are Int32 in custom.capnp: non-zero means blind
            if (sm.updated("amapNavi")) {
                last_cam_left = amap.getLeftBlind() != 0;
                last_cam_right = amap.getRightBlind() != 0;
                last_cam_left_t = sm.rcv_time("amapNavi");
                last_cam_right_t = sm.rcv_time("amapNavi");

                // Debug log: print when amapNavi message is updated
                uint64_t rcv = last_cam_left_t;
                double age_s = (nanos_since_boot() - rcv) / 1e9;
                std::cerr << "[UI][amapNavi] updated left=" << (last_cam_left?1:0)
                          << " right=" << (last_cam_right?1:0)
                          << " age=" << age_s << "s frame=" << sm.rcv_frame("amapNavi")
                          << std::endl;
            }
        }

        // Effective blind flags: prefer camera-held state (within hold window), otherwise fallback to carState
        double now_s = nanos_since_boot() / 1e9;
        bool effective_left = left_blindspot;
        bool effective_right = right_blindspot;
        if (last_cam_left && (now_s - (last_cam_left_t / 1e9) < CAMERA_HOLD_S)) effective_left = true;
        if (last_cam_right && (now_s - (last_cam_right_t / 1e9) < CAMERA_HOLD_S)) effective_right = true;
...
public:
    ...
    int     blink_timer = 0;
    int     disp_timer = 0;
    static constexpr int kDispPageTicks = 200;
    static constexpr int kDispCycleTicks = kDispPageTicks * 2;
    float cpuTemp = 0.0f;
    float cpuUsage = 0.0f;
    int   memoryUsage = 0;
    int   gpuUsage = 0;
    ...
    void drawHud(UIState* s) {
        ...
        disp_timer = (disp_timer + 1) % kDispCycleTicks;
        nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_BOTTOM);
        ...
        if (show_device_state) {
            char str[128];
            bool rotate_page_a = (disp_timer < kDispPageTicks);
            dx = bx - 35;
            dy = by - 200;
            mode_color = COLOR_GREEN_ALPHA(190);
            int cpu_warn = rotate_page_a ? (cpuTemp > 80) : (cpuUsage > 85);
            ui_fill_rect(s->vg, { dx - 65, dy - 38, 130, 90 }, (cpu_warn && blink_timer <= 8) ? COLOR_RED : mode_color, 15, 2);
            ui_draw_text(s, dx, dy - 5, "CPU", 25, COLOR_WHITE, BOLD);
            if (rotate_page_a) sprintf(str, "%.0f\u00B0C", cpuTemp);
            else sprintf(str, "%.0f%%", cpuUsage);
            ui_draw_text(s, dx, dy + 40, str, 40, COLOR_WHITE, BOLD);

            dx += 150;
            int mem_warn = rotate_page_a ? (memoryUsage > 85) : (gpuUsage > 85);
            ui_fill_rect(s->vg, { dx - 65, dy - 38, 130, 90 }, (mem_warn && blink_timer <= 8) ? COLOR_RED : mode_color, 15, 2);
            ui_draw_text(s, dx, dy - 5, rotate_page_a ? "MEM" : "GPU", 25, COLOR_WHITE, BOLD);
            if (rotate_page_a) sprintf(str, "%d%%", memoryUsage);
            else sprintf(str, "%d%%", gpuUsage);
            ui_draw_text(s, dx, dy + 40, str, 40, COLOR_WHITE, BOLD);
    ...
    void drawDateTime(const UIState* s) {
        ...
        if (show_datetime) {
            ...
            if (show_datetime == 1 || show_datetime == 3) {
                ...
                const char* weekdays_zh[] = { "日", "一", "二", "三", "四", "五", "六" };
                strftime(str, sizeof(str), "%m-%d", local); // 날짜만 가져옴
                int weekday_index = local->tm_wday; // tm_wday: 0=日, 1=一, ..., 6=六
                snprintf(str + strlen(str), sizeof(str) - strlen(str), "(%s)", weekdays_zh[weekday_index]);
    ...
    void makeDeviceInfo(const UIState* s) {
        ...
        gpuUsage = std::max<int>(0, deviceState.getGpuUsagePercent());
...
public:
    ...
    void draw(UIState *s, int w, int h, NVGcolor bg, NVGcolor bg_long) {
        ...
        str.sprintf("LD[%.0f%%,%.2f],LT[%.0f%%,%s](%.2f/%.2f), SR(%.1f,%.1f),SO(%.1f/%.1f)",
            ...
            live_params.getAngleOffsetAverageDeg(), live_params.getAngleOffsetDeg());
...
void ui_draw_border(UIState* s, int w, int h, QColor bg, QColor bg_long) {
    ...
    if (s->vg_border == nullptr) {
        ...
        std::pair<const char*, const char*> fonts[] = {
            {"KaiGenGothicKR-Bold", "../assets/fonts/KaiGenGothicKR-Bold.ttf"},
        };
