...
class StartupAlert(Alert):
  def __init__(self, alert_text_1: str, alert_text_2: str = "随时准备接管", alert_status=AlertStatus.normal):
    alert_size = AlertSize.mid
    if HARDWARE.get_device_type() == 'mici':
      if alert_text_2 == "随时准备接管":
        alert_text_2 = ""
...
def below_engage_speed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return NoEntryAlert(f"驾驶速度超过 {get_display_speed(CP.minEnableSpeed, metric)} 后启用")
...
def paramsd_invalid_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  if not sm['liveParameters'].angleOffsetValid:
    ...
    title = "转向比不匹配"
    text = f"转向机几何结构可能有误 (比值: {steer_ratio:.1f})"
...
def personality_changed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  personality_map = {"aggressive": "运动", "standard": "标准", "relaxed": "轻松", "moreRelaxed": "更轻松"}
  personality_cn = personality_map.get(personality, str(personality).title())
  return NormalPermanentAlert(f"驾驶模式: {personality_cn}", duration=1.5)


def invalid_lkas_setting_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  text = "切换原厂LKAS以启用"
  if CP.brand == "tesla":
    text = "切换到Traffic-Aware巡航控制以启用"
  elif CP.brand == "mazda":
    text = "开启车辆自带LKAS以启用"
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.longitudinalManeuver: {
    ...
    ET.PERMANENT: NormalPermanentAlert("纵向机动模式",
                                       "确保前方道路畅通"),
  },

  EventName.lateralManeuver: {
    ET.WARNING: longitudinal_maneuver_alert,
    ET.PERMANENT: NormalPermanentAlert("横向机动模式"),
  },
  ...
  EventName.aeb: {
    ...
    ET.NO_ENTRY: NoEntryAlert("AEB：有碰撞风险"),
  },
  ...
  EventName.stockAeb: {
    ET.PERMANENT: Alert(
      ...
      AlertStatus.critical, AlertSize.full,
      Priority.HIGHEST, VisualAlert.fcw, AudibleAlert.stopStop, 2.),
    ET.NO_ENTRY: NoEntryAlert("原厂AEB：有碰撞风险"),
  },

  EventName.stockLkas: {
    ET.NO_ENTRY: NoEntryAlert("原厂LKAS：检测到车道偏离"),
  },
  ...
  EventName.brakeHold: {
    ET.NO_ENTRY: NoEntryAlert("自动驻车已激活"),
    ET.WARNING: Alert(
      "按下恢复键退出刹车保持",
      "",
      AlertStatus.userPrompt, AlertSize.small,
  ...
  EventName.parkBrake: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert("已拉起手刹"),
  },

  EventName.pedalPressed: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert("已踩踏板",
  ...
  EventName.resumeBlocked: {
    ET.NO_ENTRY: NoEntryAlert("按下设置键启动"),
  },
  ...
  EventName.excessiveActuation: {
    ET.SOFT_DISABLE: soft_disable_alert("过度作动"),
    ET.NO_ENTRY: NoEntryAlert("过度作动"),
  },
  ...
  EventName.selfdrivedLagging: {
    ...
    ET.NO_ENTRY: NoEntryAlert("驾驶进程延迟：重启设备"),
  },
  ...
  EventName.accFaulted: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("巡航错误：重启车辆"),
  ...
  EventName.controlsMismatch: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("控制系统不匹配"),
    ET.NO_ENTRY: NoEntryAlert("控制系统不匹配"),
  },

  # Sometimes the USB stack on the device can get into a bad state
  # causing the connection to the panda to be lost
  EventName.usbError: {
    ET.SOFT_DISABLE: soft_disable_alert("USB错误：重启设备"),
    ET.PERMANENT: NormalPermanentAlert("USB错误：重启设备"),
    ET.NO_ENTRY: NoEntryAlert("USB错误：重启设备"),
  },
  ...
  EventName.canError: {
    ...
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("未知车辆型号"),
    #ET.PERMANENT: Alert(
    # "未知车辆型号",
    #  "",
    #  AlertStatus.normal, AlertSize.small,
    #  Priority.LOW, VisualAlert.none, AudibleAlert.none, 1., creation_delay=1.),
    ET.NO_ENTRY: NoEntryAlert("未知车辆型号"),
  },
  ...
  EventName.steerUnavailable: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("LKAS故障：重启车辆"),
    ET.PERMANENT: ImmediateDisableAlert("LKAS故障：重启车辆以启用"),
    ET.NO_ENTRY: NoEntryAlert("LKAS故障：重启车辆"),
  },
  ...
  EventName.reverseGear: {
    ...
    ET.SOFT_DISABLE: SoftDisableAlert("倒车档位"),
  ...
  EventName.speedTooHigh: {
    ...
    ET.NO_ENTRY: NoEntryAlert("减速以启动"),
  },
