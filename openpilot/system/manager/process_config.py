...
# Legacy Qt UI (ported from cp-pc). QT=1 forces the Qt UI, raylib=1 forces the
# Python (raylib) UI; otherwise the QtUiEnabled param forces it on, otherwise it
# is the default UI on PC only.
_qt_ui_params = Params()


def _use_qt_ui() -> bool:
  if os.getenv("QT") == "1":
    return True
  if os.getenv("raylib") == "1":
    return False
  if _qt_ui_params.get_bool("QtUiEnabled"):
    return True
  return PC


USE_QT_UI = _use_qt_ui()
...
procs = [
  ...
  # Legacy Qt UI and Python (raylib) UI share the "ui" name, so exactly one
  # entry may exist depending on USE_QT_UI.
  *(  [NativeProcess("ui", "openpilot/selfdrive/ui", ["./ui"], always_run)]
  ...
  # 盲区摄像头（电子后视镜）：USB 采集 + YOLOv5 推理 + VisionIPC 推流
  NativeProcess("cam_blindspot", "camera", ["./cam_blindspot"], only_onroad, enabled=PC),
]
