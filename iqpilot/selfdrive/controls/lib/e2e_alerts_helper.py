...
class E2EAlertsHelper:
  def __init__(self):
    ...
    self.green_light_alert_enabled = False
    self.lead_depart_alert = False
    self.lead_depart_alert_enabled = False
  ...
  def _read_params(self) -> None:
    # Feature removed: keep both E2E alert chimes disabled.
    self.green_light_alert_enabled = False
    self.lead_depart_alert_enabled = False
  ...
  def update(self, sm: messaging.SubMaster, events_iq: IQEvents) -> None:
    self._read_params()
    ...
    if self.green_light_alert or self.lead_depart_alert:
      events_iq.add(custom.IQOnroadEvent.EventName.e2eChime)
