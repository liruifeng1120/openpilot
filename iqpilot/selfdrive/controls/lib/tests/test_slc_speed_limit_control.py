...
def test_slc_vcruise_applies_target_without_increasing_cruise():
  ...
  out = slc.update(long_control_active=True, now=None, time_validated=True, v_cruise=v_cruise, v_ego=27.0, sm=sm)
...
def test_slc_vcruise_show_only_does_not_modify_cruise():
  ...
  out = slc.update(long_control_active=True, now=None, time_validated=True, v_cruise=v_cruise, v_ego=26.0, sm=sm)
