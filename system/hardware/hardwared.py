...
def touch_thread(end_event):
  # PC 环境没有触摸屏设备，直接返回
  if PC:
    cloudlog.info("touch_thread skipped on PC platform")
  ...
  while not end_event.is_set():
    ...
    if (count % int(10. / DT_HW)) == 0:
      try:
        ...
        # Log modem version once (skip on PC platform)
        if AGNOS and (modem_version is None) and not PC:
        ...
        # PC 环境跳过 modem 配置
        if not PC and not modem_configured and HARDWARE.get_modem_version() is not None:
...
def hardware_thread(end_event, hw_queue) -> None:
  ...
  while not end_event.is_set():
    ...
    if sm.updated['pandaStates'] and len(pandaStates) > 0:
      ...
      # Setup fan handler on first connect to panda (PC 环境不需要风扇控制)
    ...
    # PC 环境没有 PMIC 温度传感器，需要处理空列表
    if not PC and len(msg.deviceState.pmicTempC) > 0:
...
def main():
  ...
  # TICI 和 PC 环境都可能需要 touch_thread，但 PC 中会直接跳过
  if TICI or PC:
