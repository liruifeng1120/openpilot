...
class NetworkUI(Widget):
  ...
  def _cycle_panel(self):
    esim_supported = self._esim_panel.is_supported()
    if self._current_panel == PanelType.WIFI and esim_supported:
  ...
  def _render(self, _):
    ...
    esim_supported = self._esim_panel.is_supported()
    if self._current_panel == PanelType.WIFI:
      self._nav_button.text = tr("eSIM") if esim_supported else tr("Advanced")
