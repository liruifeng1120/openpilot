...
CACHE_DIR = Path("/data/scons_cache_cp" if AGNOS else "/tmp/scons_cache_cp")
...
def build(spinner: Spinner, dirty: bool = False, minimal: bool = False) -> None:
  ...
  for n in (nproc, nproc/2, 1):
    ...
    scons: subprocess.Popen = subprocess.Popen(["scons", "-j1", "--cache-populate", *extra_args], cwd=BASEDIR, env=env, stderr=subprocess.PIPE)
...
def execute_update_script_if_exists():
  ...
  script_pattern = "op_byd_c3cp_*_delta.sh"
