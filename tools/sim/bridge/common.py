...
class SimulatorBridge(ABC):
  ...
  def __init__(self, dual_camera, high_quality, car_brand):
    set_params_enabled()
    self.params = Params()
  ...
  def print_status(self):
    # 只在非测试模式下打印状态，并且控制打印频率
    if not self.test_run and self.rk.frame % 50 == 0:  # 降低打印频率
      # 检查是否启用了调试输出
      if os.environ.get('SIM_DISABLE_DEBUG', '0') != '1':
        print("State:")
        print(f"Ignition: {self.simulator_state.ignition} Engaged: {self.simulator_state.is_engaged}")
  ...
  def _run(self, q: Queue):
    ...
    self.simulated_car = SimulatedCar(self.car_brand)
