...
try:
    ...
    libc = None  # 提供默认值以防导入失败
    io_uring = None  # 提供默认值以防导入失败
...
class DiskDevice(Compiled):
    ...
    def __init__(self, device:str):
        if LIBC_AVAILABLE and IO_URING_AVAILABLE and not DiskDevice._tried_io_uring_init: 
            self._iouring_setup()
    ...
    def _might_open(self, size):
        ...
        if sys.platform != "win32" and filename.startswith("shm:") and _posixshmem is not None:
            fd = _posixshmem.shm_open("/"+filename[4:].lstrip("/"), os.O_RDWR, 0o600)
            self.mem = mmap.mmap(fd, self.size, mmap.MAP_SHARED | MAP_POPULATE | MAP_LOCKED)  # type: ignore
            os.close(fd)
        else:
            ...
            if self.fd is not None and self.size is not None:
                if os.fstat(self.fd).st_size < self.size: os.ftruncate(self.fd, self.size)
                self.mem = mmap.mmap(self.fd, self.size)  # type: ignore
    ...
    def _iouring_setup(self):
        ...
        # 只有在 libc 可用且平台支持时才尝试初始化 io_uring
        if not LIBC_AVAILABLE or not IO_URING_AVAILABLE or sys.platform != 'linux' or hasattr(sys, "getandroidapilevel"):
            return
            
        if libc is None or not hasattr(libc, 'syscall') or io_uring is None:
            return
        ...
        if libc is not None and io_uring is not None:
            fd = libc.syscall(io_uring.NR_io_uring_setup, 4096, ctypes.byref(p:=io_uring.struct_io_uring_params()))
            if fd < 0: return

        # 确保变量已初始化后再使用
        if libc is not None and io_uring is not None and p is not None and fd >= 0:
        ...
        if libc is not None and io_uring is not None and p is not None and fd >= 0 and sq_ptr is not None and cq_ptr is not None and sqes is not None:
